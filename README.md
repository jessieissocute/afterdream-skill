...
Claude Code 的 autodream 是整理记忆（NREM）——归档、去重、压缩。这个 Skill不一样，它补上的是另一半：做梦（REM）——关掉逻辑过滤器，让远距离的记忆碎片自由碰撞，发现你和 Agent 都没注意到的底层关联。
...
Agent 的记忆能告诉它发生了什么。但它不会把两件看起来毫不相关的事连起来，做更泛化的联想和贯通，无法connecting the dots。

但人在做梦的时候会，一个冷知识：门捷列夫元素周期表就是做梦的时候发现的。
