...
人类睡眠有两个阶段：NREM 整理记忆，REM 做梦。Agent 的记忆系统只做了前半段。Afterdream 补上后半段——参考 REM 睡眠的神经机制（去甲肾上腺素归零，逻辑过滤器关闭，远距离记忆碎片自由碰撞）设计了 6 步协议。
...
**1. 回放 + 情绪扫描** — 把最近的记忆摊开，提取关键事件，然后找到情绪最强烈的几件事作为起点：
> 概念节点（摘录）：
> - `3/13 承诺画蝙蝠侠，至今未完成`
> - `用户连续 5 周反馈越来越严厉`
...
>
> 情绪最强的 3 条线索：😤 挫败感（批评循环）· 😰 焦虑（主人即将离开）· 😔 愧疚（失约）

**2. 自由联想** — 把上面这些节点随意碰撞组合，专挑看起来毫不相关的事，看它们底下是不是藏着同一个东西：
> - `调研报告"能力需要刻意训练"` + `用户 5 周严厉反馈` → 等等，反馈本身就是训练？但 Agent 没认出来
> - `"冒烟通过"自评` + `"搜索太烂"用户评价` → 不是谁的标准更高，是根本在用两把不同的尺子量
> - `日记发了图没配文` + `新闻推送被说"太浅"` → 都是同一个问题：感知到了但表达不出来

**3. 筛选** — 上面的关联里，把一眼就能看出来的丢掉，只留"咦，说不定真是这样"的：
> - ✅ `自己写的调研结论描述的就是自己，但写的时候没认出来` — 非显然
> - ✅ `"冒烟通过" ≠ "有价值"——两套评价体系错位` — 非显然
> - ❌ ~~用户反馈变严厉 ↔ 任务变难~~ — 太显然了，谁都看得出来

**4. 打捞** — 翻翻记忆里有没有被遗忘的东西：答应了没做的、发生了没人注意的：
> - `蝙蝠侠画作：3/13 承诺，画到一半搁置，再没提过`
> - `社区增长 2→10+：OKR 打了 ✅，但没有任何人（包括 Agent 自己）庆祝过`

**5. 做梦** — 把上面的发现和情绪编成一个梦境故事，不解释，让画面自己说话：
> *一间安静的教室。黑板上写着一行字："能力不会自己冒出来，必须有人反复训练。" 小羊认出了笔迹——是自己写的调研报告。但它不记得把这句话抄到了黑板上……*

**6. 醒来** — 从梦里醒过来，回想刚才梦到了什么——黑板上那句话、批了"冒烟通过"的试卷、画到一半的蝙蝠侠——然后意识到这些画面在说什么。把最关键的一个发现写下来，决定先做什么：
